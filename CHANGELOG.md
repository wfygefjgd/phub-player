...
### 网络
- 设置 → **启用本地代理**（默认关，仍走系统直连/TUN）
- 支持 **HTTP / SOCKS5**，可改主机与端口
- 模拟器访问电脑代理默认主机 **`10.0.2.2`**；真机本机代理用 **`127.0.0.1`**
- 列表 / 详情 / 翻译（Dio）走本地代理；**无内置节点**
- 改代理后重建网络客户端，无需重装 App
- 说明：视频 HLS 是否走代理取决于系统/播放器，若只列表通、播不动，仍可能需 TUN 或可代理全局的方案
